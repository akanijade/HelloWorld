...
void unorderedLinkedList<Type>::deleteAll(const Type& deleteItem)
{
    ...
    prev = this->first;
    walker = this->first->link;
    while(walker != NULL){
        if(walker->info != deleteItem){
            prev = walker;
            walker = walker->link;
        }
        else{
...
void unorderedLinkedList<Type>::deleteSmallest()
{
    // TBA
    nodeType<Type> *walker;
    nodeType<Type> *prevWalker;
    nodeType<Type> *smallestVal;
    nodeType<Type> *prevIndexBeforeSV; //SV = Smallest Value
    if(this->isEmptyList()){
        std::cout << "the list is empty" << std::endl;
        return; 
    }
    else if(this->length() == 1){ //meaning only one node
        walker = this->first;
        this->first = this->first->link;
        delete walker;
        return;
    }
    smallestVal = this->first;
    prevIndexBeforeSV = NULL;
    prevWalker = this->first;
    walker = this->first->link;
    do{
        if(smallestVal->info > walker->info){
            smallestVal = walker;
            prevIndexBeforeSV = prevWalker;
        }
        prevWalker = walker;
        walker = walker->link;
    }while(walker != NULL);
    
    if(smallestVal == this->first){ //if the first node is the smallest
        this->first = this->first->link;
        this->count--;
        delete smallestVal;
        return;
    ...
    prevIndexBeforeSV->link = smallestVal->link;
    if(smallestVal == this->last)
        this->last = prevIndexBeforeSV;
    delete smallestVal;
}
